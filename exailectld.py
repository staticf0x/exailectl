...
def exaile_cmd(cmd):
    prc = Popen(
        "qdbus org.exaile.Exaile /org/exaile/Exaile " + cmd, shell=True, stdout=PIPE
    )
...
def get_track_info():
    return json.dumps(
        {
            "title": exaile_cmd("GetTrackAttr title"),
...
def get_state():
    ...
    return json.dumps({"state": state, "progress": progress})
...
def playlist():
    ...
    for line in lines[::2]:
        ...
        if m:
            ...
            tracks.append({"title": title, "artist": artist, "length": length_info})
